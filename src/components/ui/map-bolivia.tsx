	...
	return (
		<div className="map-root">
			<svg
				...
				className="map-svg"
			>
				<title id="map-title">Mapa de Bolivia por departamentos</title>
				<g className="map-features">
					{departments.map((department) => {
						const targetId = targetSections[department.name];
						return (
							<path
								key={department.id}
								d={department.d}
								className={`department${targetId ? " department--link" : ""}`}
								data-name={department.name}
								tabIndex={targetId ? 0 : -1}
								role={targetId ? "button" : undefined}
								aria-label={targetId ? `Ver candidatos de ${department.name}` : undefined}
				...
				<g className="map-labels">
					{departments.map((department) => (
						<text
							key={`${department.id}-label`}
							x={department.label.x}
							y={department.label.y}
